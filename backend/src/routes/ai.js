...
const { callOpenAI, estimateTokensForText, redactPII } = require('../services/aiService');
const orgRateLimiter = require('../middleware/orgRateLimiter');

// POST /api/ai/revenue
// Body: { task: 'pricing'|'copy'|'email', eventId?, prompt? }
router.post('/revenue', requireAuth, orgRateLimiter, async (req, res, next) => {
  ...
  try{
    ...
    // Call OpenAI (service itself applies redaction again before sending)
    const ai = await callOpenAI({ systemPrompt, userPrompt, maxTokens:800 });

    // Persist usage (store redacted prompt for privacy)
    try{
      await client.query(
        `INSERT INTO ai_usage (company_id, event_id, task, prompt_snippet, response_snippet, tokens_used, created_at)
         VALUES ($1,$2,$3,$4,$5,$6, NOW())`,
        [companyId || null, eventId || null, task, redactedPrompt.slice(0,500), ai.text.slice(0,2000), (ai.usage && ai.usage.total_tokens) || null]
      );
